...
export default function Account() {
  ...
  const handleLogin = (e: React.FormEvent) => { e.preventDefault(); setErr(''); if (!login(loginForm.email, loginForm.password)) setErr(txt.loginErr); };
  const handleRegister = (e: React.FormEvent) => { e.preventDefault(); setErr(''); const store: Record<string, { password: string; profile: UserProfile }> = JSON.parse(localStorage.getItem('motoriva_users') || '{}'); if (store[regForm.email.toLowerCase()]) { setErr(txt.regErr); return; } const ok = register(regForm.email, regForm.password, regForm.firstName, regForm.lastName); if (!ok) { setErr(txt.regErr); return; } updateProfile({ street: regForm.street, zip: regForm.zip, city: regForm.city, country: regForm.country }); };
  const handleSave = (e: React.FormEvent) => { e.preventDefault(); updateProfile(profile); setSaved(true); setTimeout(() => setSaved(false), 2500); };
