...
function CountryField({ label, value, onChange, lang }: CountryFieldProps) {
  const selectCls = 'mt-2 block h-12 w-full border border-[#333] bg-[#111] px-4 text-sm text-white outline-none focus:border-[#e02020] transition';
  return (
    <label className="block text-sm text-gray-300">
      {label}
      <select value={value || 'CH'} onChange={e => onChange(e.target.value)} className={selectCls}>
...
export default function Account() {
  ...
  const [regForm, setRegForm] = useState({ email: '', password: '', firstName: '', lastName: '', street: '', zip: '', city: '', country: 'CH' });
